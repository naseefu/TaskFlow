...
const TaskBoard = ({ isDashboard,isAllTask,setTaskLength }) => {
...
const getMonthName = (monthNumber)=> {
  ...
  return (
    <div className='taskboard'>
      ...
      >
        {tasks.filter((task)=>task.duration!="Expired").length > 0 ? (
          tasks.filter((task)=>task.duration!="Expired").map((task, index) => (
            <div className='task1-2-card1' key={index}>
              ...
              <div className='card-lefttime' style={{display:'flex',alignItems:'center',justifyContent:'space-between'}}>
